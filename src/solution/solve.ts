...
const rotateLeft = function (array, l, r) {
	const tmp = array[l]
	for (let i = l, end = r - 1, asc = l <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
		array[i] = array[i + 1]
	}
	return (array[r] = tmp)
}
...
const rotateRight = function (array, l, r) {
	const tmp = array[r]
	for (let i = r, end = l + 1, asc = r <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
		array[i] = array[i - 1]
	}
	return (array[l] = tmp)
}
...
const permutationIndex = function (context, start, end, fromEnd) {
	...
	return function (index) {
		...
		if (index != null) {
			...
			let asc3, end4
			let c
			for (i = 0, end2 = maxOur, asc1 = 0 <= end2; asc1 ? i <= end2 : i >= end2; asc1 ? i++ : i--) {
				our[i] = i + start
			}
			...
			perm = this[permName]
			for (i = 0, end3 = maxAll, asc2 = 0 <= end3; asc2 ? i <= end3 : i >= end3; asc2 ? i++ : i--) {
				perm[i] = -1
			}
			...
			let asc9
			perm = this[permName]
			for (i = 0, end6 = maxOur, asc6 = 0 <= end6; asc6 ? i <= end6 : i >= end6; asc6 ? i++ : i--) {
				our[i] = -1
			}
			a = b = x = 0
	...
	for (var tableName of Array.from(tables)) {
		...
		if (tableName === "mergeURtoDF") {
			this.moveTables.mergeURtoDF = (() =>
				__range__(0, 335, true).map((URtoUL) => __range__(0, 335, true).map((UBtoDF) => mergeURtoDF(URtoUL, UBtoDF))))()
		} else {
